...
def main():
    ...
    # ------------------------------------------------------------
    # Obstacle (solid box) definition: "spicy" lid-driven cavity
    # ------------------------------------------------------------
    solid = np.zeros_like(X, dtype=bool)

    # A centered rectangle obstacle (tweak these to move/resize)
    # Keep it away from the walls to avoid extra boundary headaches.
    i0, i1 = int(0.40 * (N_POINTS-1)), int(0.60 * (N_POINTS-1))
    ...
    for _ in tqdm(range(N_ITERATIONS)):
        ...
        # ------------------------------------------------------------
        # Enforce obstacle again after pressure correction
        # ------------------------------------------------------------
