...
def save_frame(X, Y, p, u, v, x0, y0, width, height, iteration, current_time):
    ...
    contour = ax.contourf(X[::2, ::2], Y[::2, ::2], p[::2, ::2], cmap="coolwarm")
    fig.colorbar(contour, ax=ax)

    ax.quiver(X[::2, ::2], Y[::2, ::2], u[::2, ::2], v[::2, ::2], color="black")
    ...
    plt.savefig(f"frames/frame_{iteration:04d}.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
